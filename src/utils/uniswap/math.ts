...
import { PoolAPIResponse, Position } from "../../types";
...
export const calculatePositionFees = (
  ...
  position: Position,
  token0: {decimals: number;} | null,
  token1: {decimals: number;} | null
) => {
  ...
  const feeGrowthGlobal_0 = new bn(pool.feeGrowthGlobal0X128);
  const feeGrowthGlobal_1 = new bn(pool.feeGrowthGlobal1X128);

  // Fee growth outside '𝑓𝑜' of our lower tick for both token 0 and token 1
  const tickLowerFeeGrowthOutside_0 = new bn(
    position.tickLower
  );
  const tickLowerFeeGrowthOutside_1 = new bn(
    position.tickLower
  );

  // Fee growth outside '𝑓𝑜' of our upper tick for both token 0 and token 1
  const tickUpperFeeGrowthOutside_0 = new bn(
    position.tickUpper
  );
  const tickUpperFeeGrowthOutside_1 = new bn(
    position.tickUpper
  );
  ...
  // for both token 0 and token 1
  const fr_t1_0 = feeGrowthGlobal_0
    .minus(tickLowerFeeGrowthBelow_0)
    .minus(tickUpperFeeGrowthAbove_0);
  const fr_t1_1 = feeGrowthGlobal_1
    .minus(tickLowerFeeGrowthBelow_1)
    .minus(tickUpperFeeGrowthAbove_1);

  // '𝑓𝑟(𝑡0)' part of the '𝑓𝑢 =𝑙·(𝑓𝑟(𝑡1)−𝑓𝑟(𝑡0))' formula
  // for both token 0 and token 1
  const feeGrowthInsideLast_0 = new bn(position.feeGrowthInside0LastX128);
  const feeGrowthInsideLast_1 = new bn(position.feeGrowthInside1LastX128);
  ...
  const uncollectedFees_0 = mulDiv(
    liquidity,
    fr_t1_0.minus(feeGrowthInsideLast_0),
    Q128
  );
  const uncollectedFees_1 = mulDiv(
    liquidity,
  ...
  const uncollectedFeesAdjusted_0 = uncollectedFees_0.div(
    expandDecimals(1, Number(token0?.decimals || 18)).toFixed(
      Number(token0?.decimals || 18)
    )
  );
  const uncollectedFeesAdjusted_1 = uncollectedFees_1.div(
...
// export const getLiquidityFromTick = (poolTicks: {
//   liquidity: string;
// }[], tick: number): bn => {
...
//     liquidity = liquidity.plus(new bn(poolTicks[i].liquidity));
